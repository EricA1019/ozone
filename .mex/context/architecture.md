...
User runs ozone -> splash -> launcher dashboard -> pick model -> planner computes settings -> confirm -> launches KoboldCpp -> polls API -> opens SillyTavern. Monitor mode shows live VRAM/RAM/CPU/services with 2s refresh.
...
- ui/: ratatui TUI (mod.rs, launcher.rs, monitor.rs, splash.rs).
