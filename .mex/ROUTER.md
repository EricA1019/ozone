...
- PTY launcher smoke remains marker-based and imperfect, but the old empty-sandbox failure mode is reduced now that `mock_user_tool` / `screenshot_tool` can auto-provision the recommended temp-XDG setup; if launcher handoff still fails after that, treat it as a real launcher-path bug instead of a missing test fixture.
