...
impl ShellState {
    ...
    pub(crate) fn scroll_conversation(&mut self, layout: &crate::layout::LayoutModel, delta: isize) {
        ...
        let next = current
            ...
            .min(viewport.max_scroll);
        self.conversation_scroll = if next == viewport.default_scroll_offset {
            None
        } else {
            Some(next)
        };
    ...
    pub(crate) fn submit_draft(&mut self) {
        ...
        if let Some(path) = parse_attach_command(&prompt) {
            match read_file_for_attach(&path) {
                Ok(content) => {
                    let filename = path
                        .file_name()
                        .and_then(|n| n.to_str())
                    ...
                    self.draft = DraftState::with_text(&attachment);
                    ...
                    self.focus = FocusTarget::Draft;
                    self.input_mode = InputMode::Insert;
                    self.status_line = Some(format!("Attached {filename}— edit or press Enter to send"));
                    self.show_toast(&format!("📎 {filename} attached"));
                }
                Err(err) => {
                    self.status_line = Some(format!("Attach failed: {err}"));
                }
            }
            return;
        }
