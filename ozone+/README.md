...
- **Email:** [ScribeALB@proton.me](mailto:ScribeALB@proton.me)

---

## 10. Current Status

**ozone+ is fully implemented through Phase 3 and ships as `v0.4.8-alpha`.**

Today `ozone+` ships in local family installs as the `ozone-plus` binary. The
base `ozone` launcher can also hand off into it when the local family install
is in sync. GitHub Releases still publish the base `ozone` artifact only.
...
| `/memory list` | List saved pinned-message and note memories |
| `/memory unpin <id>` | Remove a saved memory |
| `/search session <query>` | Search this session's messages plus saved memory text |
| `/search global <query>` | Search messages plus saved memory text across all sessions |
