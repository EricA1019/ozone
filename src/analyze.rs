...
fn export_presets_conf_impl(conf_path: &Path, model: Option<&str>, noisy: bool) -> Result<usize> {
    ...
    let profiles_to_export: Vec<ProfileRow> = match model {
        ...
        None => {
            ...
            let mut model_names: Vec<String> = all_bench.iter().map(|r| r.model_name.clone()).collect();
            model_names.sort();
            model_names.dedup();
    ...
    auto_lines.push(format!("# Generated by `ozone analyze --export` on {}", chrono::Local::now().format("%Y-%m-%d %H:%M")));
